...
class Members extends CI_Controller {
	...
	function read(){
    	...
    	$dat = $this->db->query("SELECT nama_reseller, nama_lengkap FROM rb_reseller a JOIN rb_konsumen b ON a.id_konsumen=b.id_konsumen where a.id_konsumen='$id'");
        $row = $dat->row();
    	$data['title'] = 'Inbox - '.$row->nama_lengkap;
		$data['row'] = $this->model_reseller->profile_konsumen($this->session->id_konsumen)->row_array();
...
Silahkan cek segera, Terima kasih.";
	...
	function otp(){
		if ($this->session->level=='konsumen'){
			...
			if (isset($_POST['submit'])){
				if ($this->session->pin==cetak($this->input->post('kode'))){
					$this->session->set_userdata(array('pin_used'=>cetak($this->input->post('kode'))));
...
The code will expire in 10 minutes, If you have not tried to login, ignore this message.";
	...
	function tolak_penjualan(){
		...
		$where = array('id_penjualan' => cetak($this->uri->segment(3)),'id_penjual'=>reseller($this->session->id_konsumen));
		$this->model_app->update('rb_penjualan', $data1, $where);
		// notif_pesanan_batal(cetak($this->uri->segment(3)));
	...
	function upgrade(){
		...
		if (isset($_GET['paket'])){
			$rowp = $this->db->query("SELECT * FROM rb_reseller_paket a JOIN rb_paket b ON a.id_paket=b.id_paket where a.id_reseller='".reseller($this->session->id_konsumen)."' AND a.paket='on'")->row_array();
			if ($rowp['status']=='Y'){
				...
				$cekp = $this->db->query("SELECT * FROM rb_reseller_paket where id_reseller='".reseller($this->session->id_konsumen)."' AND status='N' AND paket!='on'");
				...
				if ($cekp->num_rows()<='0'){
					...
					$where = array('id_reseller' => reseller($this->session->id_konsumen),'status' => 'N','paket!='=>'on');
