	...
	function keranjang(){
		...
		if (isset($_POST['update'])){
			...
			foreach ($keranjang->result_array() as $row){
				// $variasi = $this->db->query("SELECT * FROM rb_produk_variasi where id_produk='$row[id_produk]'");
				// if ($variasi->num_rows()>0){
				// 	$no = 1;
				// 	foreach ($variasi->result_array() as $va) {
				// 		$variasi = explode(";",$va['variasi']);
				// 		for ($ii=0; $ii < count($variasi) ; $ii++) { 
				// 			if (trim($this->input->post('variasi'.$no.$ii.$i)!='')){
				// 				$variasi_pilih[] = cetak($this->input->post('variasi'.$no.$ii.$i));
				// 			}
				// 		}
				// 		$no++;
				// 	}
				// 	$variasi_simpan = '||'.implode("; ",$variasi_pilih);
				// }else{
				// 	$variasi_simpan = '';
				// }
...
function selesai_belanja(){
	...
	public function detail(){
		...
		$dat = $this->db->query("SELECT * FROM rb_produk where produk_seo='$ids' AND id_reseller!='0' AND aktif='Y'");
		$row = $dat->row();
	...
	function cart(){
		...
		if ($this->session->id_konsumen!=''){
			...
			$cek_keranjang = $this->db->query("SELECT a.session, b.jenis_produk FROM `rb_penjualan_temp` a JOIN rb_produk b ON a.id_produk=b.id_produk where a.session='".$this->session->idp."' AND b.jenis_produk!='$harga[jenis_produk]'");
			if ($cek_keranjang->num_rows()<=0){
				$reseller_cek = $this->db->query("SELECT b.id_reseller FROM `rb_penjualan_temp` a JOIN rb_produk b ON a.id_produk=b.id_produk where session='".$this->session->idp."' AND b.id_reseller!='$id_reseller' GROUP BY b.id_reseller");
				if ($reseller_cek->num_rows()<'3'){
					$cek = $this->model_app->view_where('rb_penjualan_temp',array('session'=>$this->session->idp,'id_produk'=>$id_produk))->num_rows();
					...
					echo json_encode('Keranjang hanya bisa berisi 3 toko saja, silahkan hapus beberapa toko untuk menambahkan lainnya.');
				}
			}else{
				echo json_encode('Jenis produk (Digital dan Fisik) tidak bisa disatukan dalam 1 keranjang');
			}
